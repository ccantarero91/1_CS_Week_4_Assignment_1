...
object Huffman {
  ...
  // Part 1: Basics
    def weight(tree: CodeTree): Int = tree match {
      ...
      case _ => throw new Error("weight doesn't match Fork or Leaf")
    }
  
    def chars(tree: CodeTree): List[Char] = tree match {
      case f: Fork => f.chars
      case l: Leaf => List(l.char)
      case _ => throw new Error("chars doesn't match Fork or Leaf")
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      freqs.sortWith(_._2 > _._2).foldLeft(List() : List[Leaf])((z,i) => Leaf(i._1,i._2) :: z)
    }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      text.foldLeft(List() : List[Bit])((z,i) => z union codeBits(convert(tree))(i) )
    }
  }
